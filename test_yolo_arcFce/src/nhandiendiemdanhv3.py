...
# ==================== EMBEDDINGS VÀ QUẢN LÝ =====================

def load_database():
    if not os.path.exists(EMBEDDINGS_PATH):
        return [], np.array([])  # Trả về danh sách rỗng nếu không có dữ liệu embeddings
...
def show_timed_message(name):
    message = f"{name} đã điểm danh hôm nay."
    message_window = tk.Toplevel()
    message_window.title("Điểm Danh")
    tk.Label(message_window, text=message, font=("Arial", 14)).pack(pady=20)
    message_window.after(5000, message_window.destroy)  # Tắt cửa sổ sau 5 giây
